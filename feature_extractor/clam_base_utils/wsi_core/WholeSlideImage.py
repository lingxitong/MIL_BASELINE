...
class WholeSlideImage(object):
    def __init__(self, path):
        ...
        if path.endswith('.sdpc'):
            try:
                ...
                print("Please install the opensdpc package : pip install opensdpc (https://github.com/WonderLandxD/opensdpc)")
        ...
        self.level_down = self.wsi.level_downsamples
    ...
    def get_original_magnification(self):
        if self.path.endswith('.sdpc'):
            ori_magnification = self.wsi.readSdpc(self.path).contents.picHead.contents.rate
        else:
            if "aperio.AppMag" in self.wsi.properties.keys():
                ori_magnification = float(self.wsi.properties["aperio.AppMag"])
            else:
                ori_magnification = 40.0
        return ori_magnification
    ...
    def process_contours(self, save_path, real_patch_size, patch_level=0, patch_size=256, step_size=256, strict_control = False, **kwargs):
    ...
    def process_contour_strict(self, cont, contour_holes, patch_level, save_path, real_patch_size, patch_size = 256, step_size = 256,
        contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None):
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
        ...
        if len(results)>0:
            ...
            attr = {'patch_size' :            patch_size, # To be considered...
                    'real_patch_size' :       real_patch_size,
                    'patch_level' :           patch_level,
                    'downsample':             self.level_downsamples[patch_level],
                    'downsampled_level_dim' : tuple(np.array(self.level_dim[patch_level])),
                    'level_dim':              self.level_dim[patch_level],
