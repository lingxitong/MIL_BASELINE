"""
MSM_MIL - Multi-Scale Mamba-based Multiple Instance Learning
"""
import torch
...
try:
    # Try importing from local mamba package first (from MambaMIL/MSMMIL project)
    try:
        from mamba.mamba_ssm.modules.srmamba import SRMamba
        from mamba.mamba_ssm.modules.bimamba import BiMamba
        from mamba.mamba_ssm.modules.mamba_simple import Mamba
        # MultiMamba might be in a different location
        try:
            from mamba.mamba_ssm.modules.multimamba import MultiMamba
        except ImportError:
            # If MultiMamba not found, use SRMamba as fallback
            MultiMamba = SRMamba
    except ImportError:
        # Fallback to standard mamba-ssm package
        try:
            from mamba_ssm.modules.mamba_simple import Mamba
            # For SRMamba, BiMamba, MultiMamba, we may need to implement them or use Mamba
            SRMamba = BiMamba = MultiMamba = Mamba
        except ImportError:
    ...
    SRMamba = BiMamba = Mamba = MultiMamba = None
...
class MSM_MIL(nn.Module):
    """
    Multi-Scale Mamba-based Multiple Instance Learning
    Reference: MSMMIL paper
    """
    def __init__(self, in_dim=1024, num_classes=2, dropout=0.1, act=nn.ReLU(), 
                 layer=2, rate=10, mamba_type="SRMamba", **kwargs):
        super(MSM_MIL, self).__init__()
    ...
    def forward(self, x, return_WSI_attn=False, return_WSI_feature=False):
        ...
        # Apply Mamba layers
        if self.mamba_type == "SRMamba" or self.mamba_type == "MultiMamba":
            for layer in self.layers:
                h_ = h
                h = layer[0](h)  # LayerNorm
                # SRMamba/MultiMamba forward signature may vary, try with rate first
                try:
                    h = layer[1](h, rate=self.rate)  # SRMamba/MultiMamba with rate
                except TypeError:
