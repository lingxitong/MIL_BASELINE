...
def process_PGCN_MIL(args):
    ...
    hidden_dim = args.Model.hidden_dim if hasattr(args.Model, 'hidden_dim') else 256
    n_layers = args.Model.n_layers if hasattr(args.Model, 'n_layers') else 2
    k = args.Model.k if hasattr(args.Model, 'k') else 5
    use_dgl = args.Model.use_dgl if hasattr(args.Model, 'use_dgl') else False
