...
def process_MHIM_MIL(args):
    ...
    # MHIM_MIL specific parameters
    mlp_dim = args.Model.mlp_dim if hasattr(args.Model, 'mlp_dim') else 512
    head = args.Model.head if hasattr(args.Model, 'head') else 8
    mask_ratio = args.Model.mask_ratio if hasattr(args.Model, 'mask_ratio') else 0.0
    baseline = args.Model.baseline if hasattr(args.Model, 'baseline') else 'selfattn'
