...
from modules.RNN_MIL.rnn_mil import *
...
def process_RNN_MIL(args):
    ...
    for epoch in tqdm(range(args.General.num_epochs),colour='GREEN'):
        ...
        train_loss,cost_time = rnn_train_loop(device,mil_model,train_dataloader,criterion,optimizer,now_scheduler)
        val_loss,val_metrics = rnn_val_loop(device,num_classes,mil_model,val_dataloader,criterion)
        if args.Dataset.VIST == True:
            test_loss,test_metrics = val_loss,val_metrics
        else:
            test_loss,test_metrics = rnn_val_loop(device,num_classes,mil_model,test_dataloader,criterion)
        FAIL = '\033[91m'
        ENDC = '\033[0m'
        print('----------------INFO----------------\n')
