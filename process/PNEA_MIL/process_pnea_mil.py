...
def _train_pnea_mil(
    ...
    for bag, label in loader:
        ...
        optimizer.zero_grad()
        output = model(bag)
        loss = criterion(output['logits'], label) + output['sparsity_loss']
        loss.backward()
        optimizer.step()
        loss_sum += loss.item()
...
def process_PNEA_MIL(args):
    ...
    mil_model = PNEA_MIL(
        ...
        evidence_dim=args.Model.get('evidence_dim', 64),
        num_classes=num_classes,
        lambda_l1=args.Model.get('lambda_l1', 5.0),
    )
    mil_model.to(device)
    print('Model Ready!')
    ...
    for epoch in tqdm(range(args.General.num_epochs), colour='GREEN'):
        ...
        train_loss, cost_time = _train_pnea_mil(
            device,
            mil_model,
            train_dataloader,
            criterion,
