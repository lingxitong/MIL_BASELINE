...
from modules.RANKMIX_MIL.rankmix_mil import RANKMIX_MIL, rankmix_data
...
def process_RANKMIX_MIL(args):
    """Process with RankMix augmentation"""
    ...
    mil_model = RANKMIX_MIL(L=L, D=D, num_classes=num_classes, dropout=dropout, act=act, in_dim=in_dim)
    mil_model.to(device)
    
    print('Model Ready!')
    print(f'Using Mix Method: RankMix (Rank-based Mixup)')
    ...
    for epoch in tqdm(range(args.General.num_epochs), colour='GREEN'):
        ...
        train_loss, cost_time = train_loop_with_mixup(device, mil_model, train_dataloader, criterion, optimizer, now_scheduler, mixup_config, rankmix_data)
    ...
    save_last_model(args, mil_model.state_dict(), args.General.num_epochs)
